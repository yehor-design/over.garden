...
| [OVE-202](https://linear.app/overgarden/issue/OVE-202) | Structured journal `JournalDocumentV1`, Editor.js owner composers (≤10 inline photos), edit/conflict receipts, offline multi-intent sync, and SSR renderer are on `main` with exact-SHA Vercel READY; Linear Done awaits founder physical iPhone Safari checklist. | `597f301203193fdb7749f57dc2c22ea3137ebc88` | Budget freeze: Vercel `dpl_A45oWncnSHncWwnfgvDQ8LDJmKwx` READY + local smoke/lint/typecheck/full test |
...
The payload-free `owner-composer-drafts` registration remains the shared OVE-205 extension seam. OVE-202 now records `browser-backed` proof with scenario `editor-clean-locale-transition` and `proofOwner: OVE-202`. OVE-206 and OVE-207 remain `downstream-owned-real-ui` with `browserScenarioId: null` until their own closeouts.

...
## Stable Registry boundary (OVE-318)

The existing EPPO role remains a bounded, source-backed code/name
corroboration path for the planned MVP wave. It is not a full EPPO taxonomy
mirror. ADR-0016 and `docs/STABLE_REGISTRY.md` now allow OVE-254 to create a
separately labelled, immutable OverGarden observed capture after proving its
documented acquisition contract, bounded closure, digests, rights evidence,
and source-only safety. Such a capture is never an official EPPO release.

Historical OVE-253 receipt: its 2026-08-03 `blocked_manifest` result correctly
proved that the documented API did not expose an official versioned checksum
manifest or provider-owned complete-corpus closure method. That receipt remains
evidence against calling a capture an EPPO release; ADR-0016 supersedes only
the indefinite future-work wait.

This boundary does not change the current four-species wave, its reviewed
aliases, its attribution, or the prohibition on distribution/native-range
text. Captured records cannot enter identity, an immutable Foundation/Edition,
the product catalog, or search until each later gate passes independently. A
portal browser session, undocumented endpoint, or HTML scrape remains
forbidden.

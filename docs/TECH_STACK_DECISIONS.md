...
| UI                      | shadcn/ui only                                                                                                                                                                                                                                                                                        |
| Typography              | Google Sans (text) and Geist Mono (code) through `next/font/google`: subsets fetched at build time, self-hosted under `/_next/static`, preloaded, `display: swap`, hand-declared `Arial` fallback. Wired once in `apps/web/src/app/fonts.ts` and `globals.css`; no contract, verifier, or matrix (ADR-0022 D7). |

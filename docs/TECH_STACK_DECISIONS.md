...
| Object storage          | Cloudflare R2; ADR-0019 uses private ephemeral staging for direct browser upload of final WebPs and immutable public storage after atomic publication. The current private-original quarantine is transitional runtime until OVE-349/350.                                                             |
| Media processing        | Under ADR-0019, the browser-generated WebP is the sole final artifact: its exact bytes are previewed, staged, promoted, stored, and served without Vercel byte ingress or server re-encoding. OVE-347 through OVE-350 own runtime convergence and legacy retirement.                                    |

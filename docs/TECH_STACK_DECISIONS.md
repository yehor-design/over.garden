...
| Search/typeahead        | Meilisearch as a derived public index; catalog typeahead resolves in Postgres with `pg_trgm` (migration `0043`), because a derived index must not be the only thing that answers a misspelling. ADR-0026 D7 (Slice 24) takes Meilisearch off the pick path entirely: the typeahead becomes one Postgres query, prefix index then trigram                                                                                                                                                                                                                                                                 |
...
9. **Python is a worker, not a product dependency.** Typeahead is Postgres only under ADR-0026 D7 (it used Meilisearch before Slice 24). Dedup/matching can lag; SEO canonicalization must prevent thin duplicate public pages from being indexed before merge.

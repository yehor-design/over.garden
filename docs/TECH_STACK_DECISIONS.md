...
> **Current status:** consolidated by ADR-0014, superseded for connectivity and browser-local journal persistence by ADR-0017, and superseded for the MVP refusal, media, indexability, and operator-surface posture by ADR-0018. Older ADRs remain historical records; this document plus the latest explicit superseding ADR is the current stack surface for new work.
...
| Object storage          | Cloudflare R2; ADR-0018 targets format-conversion-only media. The current private-quarantine topology is transitional runtime until OVE-334.                                                                                                                                                          |
| Media processing        | WebP format conversion is the MVP contract. OVE-331 makes converted, non-revoked derivatives eligible for public projection with an explicit quality class. OVE-333/334 retire the remaining actual-byte, quarantine, metadata-strip, and original-deletion processing controls.                      |
...
2. **Media is format-conversion-only for the MVP target.** Convert accepted image input to WebP. Do not add a new quarantine-first, actual-byte, standalone metadata-strip, or original-deletion readiness promise; OVE-333 and OVE-334 own removal of current transitional controls.
3. **Encoder behavior is not a separate admission promise.** Metadata omission may occur during conversion, while client processing remains only a bandwidth optimization.

...
- Preserve a pre-versioned `BETTER_AUTH_SECRET` only for the bounded migration grace period. Better Auth reads legacy session and OAuth envelopes through its native legacy configuration; the narrow server-side verification-link bridge re-signs only a valid, unexpired legacy `/api/auth/verify-email` token with the active key, retaining its original expiry ceiling. The bridge never logs, stores, reflects, or trial-decrypts token or key material.
- Before applying a rotation, read back only Vercel target/name classes, generate independent Sensitive values for Production and Preview without printing them, and set matching non-secret version metadata. Do not replace or read the singular legacy value during the first deployment. Roll back only by removing the new versioned pair and redeploying the known-good artifact while the singular legacy fallback remains; do not remove the legacy value until a separately approved grace-window closure has continuity evidence.

...
- OVE-298 read-only Production classification on 2026-08-12 found `googleAccountRowCount=1`, zero duplicate Google-subject groups, zero duplicate Google-user groups, zero missing Google subjects, zero invalid Google-provider rows, and both approved index names absent. Migration `0022` digest is `6392a41f971176eb9de748f54fc15beb76a6a77f8a755694d327fe8eae40f6bd`; the redacted database-target digest is `84503a97fba4e9febf14db87091ce05d2866796d78109f812a649c23f9c36462`. `GOOGLE_ACCOUNT_LINKING_ENABLED` remains absent, Google credential names remain configured, deployment `dpl_HMLdqNQE6U2nxSnpQ31DvRtESYKq` remains READY/promoted at `9449455db4e4417f03ad08e7bdd4c212eb4f1f00`, and no production row, index, environment value, callback, provider grant, or session was changed. Before enablement, OVE-298 must also preserve OVE-314 by denying the sealed owner in both account-method projection and pre-provider-state server admission; its approved live proof uses exactly one ordinary non-owner/non-admin disposable signup, verification, link/unlink/revocation, and canonical erasure lifecycle.

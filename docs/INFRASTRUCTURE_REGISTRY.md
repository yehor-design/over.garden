...
- OVE-295 separates explicit Google account linking from ordinary Google sign-in with the non-secret server flag `GOOGLE_ACCOUNT_LINKING_ENABLED`. Only trimmed exact `true` plus configured Google credentials enables the native link caller; absent, empty, false, or malformed values fail closed while existing connected methods remain visible. The OVE-295 production deployment must keep the flag absent/false and must not apply migration `0022` to production. OVE-298 alone owns the authorized production duplicate-group preflight, application of both partial Google unique indexes, flag enablement, live callback/link-unlink proof, rollback read-back, and disposable fixture cleanup.
- OVE-298 Production second-retry preflight on 2026-08-12 is `googleAccountRowCount=1` with zero duplicate Google-subject groups, zero duplicate Google-user groups, zero missing Google subjects, and zero invalid Google-provider rows. Migration `0022` digest is `6392a41f971176eb9de748f54fc15beb76a6a77f8a755694d327fe8eae40f6bd`; the redacted database-target digest is `84503a97fba4e9febf14db87091ce05d2866796d78109f812a649c23f9c36462`. Both approved indexes remain exact with definition digests `fed091a72b61aa8d9b9573dadeeb345dbcdea979b7173154dddac81a9fbe7dbe` and `9e68f3c994090992c14869a501de524e96135484426b03dca2211b3bcdba4e66`. The initial approved attempt ended before callback; the first retry reached its one callback only after the one-time Better Auth OAuth state expired. Both failed closed without a Google account row or accepted provider grant. `GOOGLE_ACCOUNT_LINKING_ENABLED` is absent. PR `#137` restored the signed mutation-generation boundary on the otherwise shell-excluded `/erasure` route; exact main `17dcf25970126f442bec7e8a2374444c201507ac` is READY as deployment `dpl_ALcBt5Hjyu8ZcdwfwUUE6n4drp1p` on both canonical aliases. The retry fixture is absent after canonical erasure, its request is anonymized and completed, and its inbox, secrets, environment, approval, and private-state artifacts are gone. A further callback requires exact approval of `docs/runbooks/OVE_298_PRODUCTION_GOOGLE_LINK_PROOF_RETRY_2_PLAN.md`; it creates no index, uses one fresh ordinary fixture, requires an opaque pre-authenticated provider session, and terminates without retry if callback read-back is missing or uncertain after 120 seconds. The proof must preserve OVE-314 by denying the sealed owner in both account-method projection and direct pre-provider-state server admission.

...
- Status: `open` as of 2026-07-17. The Whalebone/A1 path has recovered; Cisco Umbrella remains the incomplete protective-DNS reputation blocker. This is not an authoritative DNS drift.
...
- Two authenticated Cisco Web Reputation submissions were created on 2026-07-13, with private case IDs present for apex and `www`. Both remained `PENDING` with current `Untrusted`, suggested `Trusted`, and no resolution on 2026-07-17. This proves submission and continued escalation only, not provider resolution or user-path recovery.
- Fresh redacted evidence on 2026-07-17: `pnpm smoke:protective-dns` produced `10 pass / 4 Cisco endpoint mismatches / 0 error`; both public Cisco Umbrella resolver endpoints still return the same phishing sinkhole for apex and `www`. Exact-main GitHub CI and the matching Vercel `READY` production deployment passed; canonical apex, `www`, `/health`, and representative Bulgarian routes responded correctly; the production dependency audit found no known vulnerability; and bounded error/5xx log checks found no compromise signal.

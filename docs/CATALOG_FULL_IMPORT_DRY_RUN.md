...
OVE-102 extends the same target with Formex parser QA. It discovers the Publications Office Formex XML ZIP from the EUR-Lex XML notice, or derives the deterministic Publications Office ZIP URL from the CELEX id when EUR-Lex XML is WAF-challenged, parses the OJ-derived variety rows, and reports accepted/review-needed/rejected confidence buckets.
...
- EUR-Lex XML notice endpoint (`legal-content/EN/TXT/XML/?uri=CELEX:...`): preferred manifestation discovery artifact; checksum and byte length are preserved when it is reachable.
- Publications Office Formex XML ZIP: primary machine-readable parser artifact for Common Catalogue table rows. If the EUR-Lex XML notice is WAF-challenged with HTTP `202`, derive this ZIP URL from the CELEX id and record the XML artifact as review-needed rather than pretending the XML notice was fetched.
...
The dry-run reports row counts by supplement, species/crop, country, notifier, and confidence bucket. IASAS-only, EU Plant Variety Portal-only, CPVO/UPOV, or missing-provenance rows cannot be upgraded through this parser path; the parser accepts only official EUR-Lex or `data.europa.eu` ELI source URLs. Missing CELEX/ELI, unavailable Formex ZIP, HTML-only evidence, PDF-only evidence, or ambiguous artifact format is reported as `review_needed`; EUR-Lex XML WAF `202` remains a recorded XML artifact caveat, but does not block parsing when the CELEX-derived Publications Office ZIP is fetched and checksummed. Do not silently skip a supplement family.

...
| `0084` | `OVE-526` | The complaint procedure: `content_reports` (target, reason, reporter contact, the owner's decision) and `moderation_messages` (the outbox of the receipt, the decision and the statement of reasons). Additive; the release before it never reads them. |
| `0085` | `OVE-519` | `alter table catalog_items drop column indexable_override` — the owner's "mark indexable" switch, read by nothing since a species page is published by one rule (`catalog-publication.ts`). Destructive, as the task asks once nothing reads the column; one row held `true` (`solanum-lycopersicum`, published by its entries anyway). Apply **after** the release that no longer reads it. `first_hand_content_at` stays: `0056`'s queue functions still write and read it. |
| `0086` | `OVE-524` | An object's species and its cultivar or breed as two explicit choices: `plant_objects.species_text` (the own species, private), `variety_state` gains `own` (own cultivar text after an own species), a CHECK over the four combinations, the 10 `free_text` labels moved to `species_text`; gardener-added cultivars and breeds as shared catalogue forms (`source = 'gardener'`, the `overgarden-gardeners` snapshot, `catalog_cultivar_key`), and the per-gardener unique index of the retired cards dropped. `free_text` stays legal only for `catalog_revert_action`'s two historic label links (29.15 removes it). `0001` and `0061` guard their narrower CHECK and the dropped index for replays. Apply **before** deploying the stepper; the release before it writes only shapes the CHECK accepts. |

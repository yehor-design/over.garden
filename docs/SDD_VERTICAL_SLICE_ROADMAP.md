...
The deterministic catalog-matching queue `OVE-158` through `OVE-163` now starts from the completed OVE-158 advisory worker/read-model contract in `docs/CATALOG_MATCH_SUGGESTION_QUEUE.md`: provisional saves enqueue privacy-safe off-request matching, and operator evidence cannot mutate canonical state. Continue with OVE-159 and OVE-160 before OVE-161; OVE-162 also depends on OVE-158, and OVE-163 remains the combined matching rollout gate. OVE-170 must localize the final OVE-163 surface rather than freezing the interim English operator copy.

...
Date: 2026-06-26
Last operational update: 2026-07-17 (OVE-189 canonical local media runtime)
Owner: founder
...
The post-audit MVP closeout queue is tracked in Linear after Slice 18. OVE-189 closes its canonical local-media prerequisite: a maintainer/agent can classify a corrupt Apple Container MinIO volume, plan without mutation, salvage the fully readable user-bucket tree from an exact read-only source into an explicit new target, preserve the source, and prove actual media plus Postgres/Meilisearch/MinIO persistence across full container recreation. The binding runbook is `docs/LOCAL_MEDIA_RUNTIME_RECOVERY.md`. Downstream issues must use the recovered canonical runtime and may not reinterpret a health endpoint alone as media proof.
...
Localization foundations `OVE-164` and `OVE-165`, incremental consumer slices `OVE-166` through `OVE-170`, and the final OVE-171 completion gate are complete and must not regress. `docs/LOCALIZATION_COVERAGE_BASELINE_2026-07-14.md` records those exact-parity contracts and the localized work shipped by OVE-172 through OVE-185; `docs/LOCALIZATION_COVERAGE_WORKFLOW.md` is the extension procedure. The `ove171-v1` gate explicitly classifies all 92 current route modules, validates 22 existing copy namespaces across `uk`/`bg`/`ru`, scans authored UI/metadata literals with exact reason-coded exclusions, and reuses the 171-scenario OVE-187 plus OVE-185/186 browser matrix for 642 route/viewport checks. Thirteen owner/edge probes add 26 explicit 320/1440 checks and raise rendered locale-contract proof to 668 checks without changing the v8 fixture manifest. Future routes, states, and components must enter this gate in the same vertical slice. Because OVE-171 lands after the OVE-186 production proof, exact-SHA CI, deployment, production smoke, and protective-DNS evidence must be refreshed against the new final `main` commit.

...
Localization foundations `OVE-164` and `OVE-165` plus the incremental consumer slices `OVE-166` through `OVE-170` are completed and must not regress. `docs/LOCALIZATION_COVERAGE_BASELINE_2026-07-14.md` records those exact-parity copy contracts and the localized work already shipped by OVE-172 through OVE-185; it is the binding do-not-rebuild boundary. `OVE-171` is now the next and final machine-checkable localization gate. It consumes OVE-166 through OVE-170 directly, with the completed OVE-161 and OVE-163 catalog contracts inherited transitively, and must prove the registered route/state matrix without rebuilding localized surfaces. `OVE-186` and `OVE-188` do not block the localization gate. Because localization commits land after the OVE-186 production proof, its exact-SHA CI, deployment, production-smoke, and protective-DNS evidence must be rerun against the new final `main` commit.

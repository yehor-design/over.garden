...
Date: 2026-06-26
Last operational update: 2026-07-15 (review-gated catalog alias contract)
Owner: founder
...
Localization foundations `OVE-164` and `OVE-165` plus the trust-sensitive consumer slice `OVE-166` are completed and must not regress. `docs/LOCALIZATION_COVERAGE_BASELINE_2026-07-14.md` records those contracts and the additional localized work already shipped by OVE-172 through OVE-185; it is the binding do-not-rebuild boundary. The remaining implementation cards are incremental delta slices, not full-surface rewrites: `OVE-167` depends on `OVE-164` and the final gardener typeahead/readback contract in `OVE-161`; `OVE-168` depends on `OVE-164`, `OVE-165`, and `OVE-161`; `OVE-169` depends on `OVE-164` and `OVE-165`; and `OVE-170` depends on `OVE-164` plus the complete operator matching rollout in `OVE-163`. `OVE-171` is the final machine-checkable localization gate blocked directly by `OVE-167` through `OVE-170` and retains completed OVE-166 as a regression input, with `OVE-161` and `OVE-163` inherited transitively. `OVE-186` and `OVE-188` do not block those localization slices. If localization commits land after the OVE-186 production proof, its exact-SHA CI, deployment, production-smoke, and protective-DNS evidence must be rerun against the new final `main` commit.

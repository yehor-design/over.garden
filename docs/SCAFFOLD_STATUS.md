...
OVE-27 adds an operator production-smoke surface and live smoke contract: `/garden/pilot-smoke` checks deployed env readiness without exposing secrets, `docs/PRODUCTION_PILOT_SMOKE.md` defines the browser smoke and evidence rules, and `docs/INFRASTRUCTURE_REGISTRY.md` records the current Vercel project/deployment plus public-access blockers. OVE-31 proves the current `main` revision on the selected production pilot alias: `https://over-garden.vercel.app` runs deployment `dpl_FqkhwnHcjAvqwUoEpHGfJAch1kQh` from commit `27e95e76968b042c717b336ab99b1b5e156844a1`, with GitHub CI green and a redacted browser smoke through auth, first entry, media derivative, follow-up, public SSR, variety activation, archive-to-410, pilot-health, and pilot-smoke.
...
- Vercel project `over-garden` exists under team `team_vs3oQAk6OT4vVVvcL7Mf5m8t`; latest verified production deployment `dpl_FqkhwnHcjAvqwUoEpHGfJAch1kQh` is READY from current `main` commit `27e95e76968b042c717b336ab99b1b5e156844a1`.
- On 2026-06-28, the selected production pilot alias passed the redacted browser smoke against managed Postgres and R2: unauthenticated public routes returned OverGarden responses, the signed-in first-entry path saved, `media.over.garden` derivative readback worked, follow-up saved, public journal SSR stayed `noindex`, public variety activation saved, archive returned `410`, and operator readouts loaded without raw private evidence.
...
Continue the post-audit pilot hardening batch from Linear. After OVE-31, the strongest remaining vertical issue is journal search worker indexing or production domain/public-access closure, depending on whether the founder wants to remove the explicit search/worker degradation first or move the selected pilot alias from Vercel subdomain to `over.garden`.

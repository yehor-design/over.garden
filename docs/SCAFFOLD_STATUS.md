...
OVE-27 adds an operator production-smoke surface and live smoke contract: `/garden/pilot-smoke` checks deployed env readiness without exposing secrets, `docs/PRODUCTION_PILOT_SMOKE.md` defines the browser smoke and evidence rules, and `docs/INFRASTRUCTURE_REGISTRY.md` records the current Vercel project/deployment plus public-access blockers. OVE-28 closes the temporary operator fail-open gate: internal operator surfaces now require an explicit `CATALOG_CURATOR_USER_IDS` allowlist.
...
OVE-39 proves production durability for the pilot journal search path without weakening any privacy boundary. The worker/process-manager story is verified from real code/config and documented without secrets: Docker Compose on the worker droplet with a container restart policy and `matching`/`meili` health endpoints, plus stale-`processing` reclaim through `FOR UPDATE SKIP LOCKED` and the `WORKER_VT_SECONDS` visibility timeout with idempotent handlers. A new deterministic recovery harness (`services/matching/tests/test_worker_recovery.py`) proves that after a simulated worker restart/crash a reclaimed `journal_entry_index` and `journal_entry_unindex` job still reaches `done`, keeps exactly the public-safe OVE-36 document contract (no owner/user IDs, media keys, precise location, IPs, or user agents), is idempotent on at-least-once re-delivery, and recovers from a transient Meilisearch outage. `/garden/pilot-smoke` now carries an explicit "Backup and worker recovery" section, and managed Postgres backup/PITR for `overgarden-postgres-prod-fra1` is recorded honestly as `UNVERIFIED-NEEDS-OPERATOR` (2026-06-29) with the exact redacted dashboard/`doctl`/API verification procedure, because the sandbox has no DigitalOcean credentials. The live DO backup/PITR confirmation and the live droplet worker-restart smoke remain operator steps and a closed-pilot launch blocker until confirmed. Redacted evidence is in `docs/INFRASTRUCTURE_REGISTRY.md` and `docs/PRODUCTION_PILOT_SMOKE.md`. No restore-over-production, bulk delete, schema drop, or history rewrite was performed.
...
- `activation_started` is a bounded server-side intent event for `/garden` starts from homepage, public-variety, or direct garden sources. It stores only enum attribution and never stores raw URLs, referrers, query strings, selected display text, or journal content.
...
- Empty allowlists fail closed across catalog curation, pilot health, pilot smoke, and erasure request operator readback/actions. Local development must use an explicit Better Auth user ID in `CATALOG_CURATOR_USER_IDS`; there is no authenticated-user fallback.
...
- Live DigitalOcean Managed Postgres backup/PITR confirmation. The redacted verification procedure is recorded (OVE-39); the live check is `UNVERIFIED-NEEDS-OPERATOR` (2026-06-29) and is a closed-pilot launch blocker until an operator confirms it.
- Live production media derivative readback through the deployed app.
- Live worker restart/recovery smoke on the DigitalOcean droplet. The process-manager story is documented and recovery is proven locally (OVE-39); the live droplet restart smoke and a live restart-policy/health confirmation remain operator steps.

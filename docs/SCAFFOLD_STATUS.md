...
- Public variety pages remain `noindex` by default. OVE-18 does not promote any page into sitemap/indexable SEO inventory.
- If a catalog slug has zero safe public entries, `/variety/[slug]` returns 404 instead of an empty indexable shell.
- Thinness/promotion thresholds are intentionally deferred to OVE-19. Revisit before index promotion: minimum entry count, text richness, media diversity, region distribution, localized slug/transliteration strategy, and whether variety-region pages need stricter k-anonymity-like gates.

...
const UK_COPY = {
  ...
  checkLabels: {
    ...
    "facebook-oauth-provider": "Провайдер Facebook Login",
  },
  ...
  smokeSteps: [
    ...
    "Для Facebook Login спочатку перевірте production-gate. Якщо Meta app не підтверджено для звичайних користувачів, Facebook має бути приховано, а email/Google — доступні. Якщо `FACEBOOK_LOGIN_PUBLIC_READY=true`, реальний користувач без ролі має завершити вхід і повернутися на `/garden` без запису auth-параметрів.",
    ...
    "Пов'язуйте Facebook для наявного облікового запису лише за `FACEBOOK_LOGIN_PUBLIC_READY=true`; інакше дія має бути прихована, а email/Google — доступні. Після ввімкнення повернення через Facebook має зберегти той самий сад і дозвіл.",
    ...
    "Відкрийте `/admin` як звичайний Google/Facebook користувач і підтвердьте відмову; потім увійдіть спеціальним owner-обліковим записом з email/паролем і підтвердьте постійний доступ `admin_user_roles`.",
...
const BG_COPY: OperatorSmokeCopy = {
  ...
  checkLabels: {
    ...
    "facebook-oauth-provider": "Доставчик Facebook Login",
  },
  ...
  smokeSteps: [
    ...
    "За Facebook Login първо провери production gate. Ако Meta app не е доказано достъпно за обикновени потребители, Facebook трябва да е скрит, а имейл/Google да останат. При `FACEBOOK_LOGIN_PUBLIC_READY=true` реален потребител без роля трябва да завърши входа без запис на auth параметри.",
    ...
    "Свързвай Facebook само при `FACEBOOK_LOGIN_PUBLIC_READY=true`; иначе действието е скрито и имейл/Google остават. При включване връщането с Facebook запазва същата градина и покана.",
    ...
    "Отвори `/admin` като обикновен Google/Facebook потребител и потвърди отказ; после влез със специалния owner профил с имейл/парола и потвърди `admin_user_roles`.",
...
const RU_COPY: OperatorSmokeCopy = {
  ...
  smokeSteps: [
    ...
    "Для Facebook Login сначала проверьте production gate. Если Meta app не подтверждено для обычных пользователей, Facebook должен быть скрыт, а email/Google доступны. При `FACEBOOK_LOGIN_PUBLIC_READY=true` реальный пользователь без роли должен завершить вход без записи auth-параметров.",
    ...
    "Связывайте Facebook только при `FACEBOOK_LOGIN_PUBLIC_READY=true`; иначе действие скрыто и email/Google доступны. При включении возврат через Facebook сохраняет тот же сад и приглашение.",
    ...
    "Откройте `/admin` как обычный Google/Facebook пользователь и подтвердите отказ; затем войдите специальным owner-профилем с email/паролем и подтвердите `admin_user_roles`.",

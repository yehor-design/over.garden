...
const COPY: Record<InterfaceLocale, Copy> = {
  uk: {
    ...
    states: {
      ...
      unresolved:
        "Не вдалося безпечно видалити сховище з невизначеною прив’язкою. Його збережено без змін.",
  ...
  bg: {
    ...
    states: {
      ...
      unresolved:
        "Хранилище с неопределена връзка не можа да бъде изтрито безопасно. То е запазено без промяна.",
      cancelled:
  ...
  ru: {
    ...
    states: {
      ...
      unresolved:
        "Хранилище с неопределённой привязкой не удалось безопасно удалить. Оно сохранено без изменений.",
      cancelled:

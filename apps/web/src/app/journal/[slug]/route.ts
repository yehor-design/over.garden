...
export async function GET(
  ...
  if (lookup.status === "gone") {
    return htmlResponse(renderGonePage(lookup.entry.publicSlug), 410);
  }
  ...
  return htmlResponse(renderEntryPage(lookup.page), 200);
}

function renderEntryPage(page: PublicJournalEntryPage) {

...
describe("catalog entity-resolution QA repository", () => {
  it("builds report queries from safe catalog/source projection fields only", () => {
    const catalog = buildCatalogEntityResolutionCatalogRowsQuery(
      testDb,
    ).compile();
    const alias = buildCatalogEntityResolutionAliasCollisionRowsQuery(
      testDb,
    ).compile();
    const source = buildCatalogEntityResolutionSourceCandidateSummaryQuery(
      testDb,
    ).compile();
    const combinedSql = [catalog.sql, alias.sql, source.sql].join("\n");
  ...
  it("groups duplicate, conflict, alias, review, and blocked clusters safely", () => {
    ...
    expect(report.schemaVersion).toBe("ove89.catalogEntityResolutionQa.v1");
    ...
    expect(
      report.clusters.some(
        (cluster) => cluster.kind === "canonical_concept",
      ),
    ).toBe(true);
  ...
  it("keeps manual and blocked groups visible when alias collisions are large", () => {
    ...
    expect(
      report.clusters.some(
        (cluster) => cluster.kind === "blocked_projection",
      ),
    ).toBe(true);
    expect(
      report.summary.groups.find((group) => group.kind === "alias_collision")

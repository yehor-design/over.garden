...
describe("pilot smoke readiness", () => {
  ...
  it("keeps journal search worker proof as a manual smoke check", () => {
    const readout = buildPilotSmokeReadiness({
      env: productionLikeEnv,
      databaseProbe: { reachable: true },
      generatedAt: new Date("2026-06-27T00:00:00.000Z"),
    ...
    expect(
      ...
      severity: "manual",
      summary: expect.stringContaining("journal_entry_unindex"),
      evidence: expect.stringContaining("Do not copy indexed document content"),
    });
  });

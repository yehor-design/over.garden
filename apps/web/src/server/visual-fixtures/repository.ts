  ...
  const media = executor
    ...
    .values(
      manifest.media.map((item) => ({
        ...
        intrinsic_width: item.width,
        intrinsic_height: item.height,
        focal_x: 0.5,
        focal_y: 0.5,
        usage_role: "inline",
        document_position:
          manifest.media
            .filter((candidate) => candidate.entryId === item.entryId)
    ...
    .onConflict((oc) =>
      oc.column("id").doUpdateSet({
        ...
        intrinsic_width: sql`excluded.intrinsic_width`,
        intrinsic_height: sql`excluded.intrinsic_height`,
        focal_x: sql`excluded.focal_x`,
        focal_y: sql`excluded.focal_y`,
        usage_role: sql`excluded.usage_role`,
        document_position: sql`excluded.document_position`,
        updated_at: sql`excluded.updated_at`,
      }),
    );

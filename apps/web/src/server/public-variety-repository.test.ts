...
describe("public variety repository query contracts", () => {
  it("counts only safe public entries for global seeded or confirmed catalog identity", () => {
    ...
    expect(compiled.sql).toContain('"catalog_items"."public_slug" = $5');
    expect(compiled.sql).toContain('"catalog_items"."status" in ($6, $7)');
    ...
    expect(compiled.sql).toContain('"plant_objects"."variety_state" = $8');
    expect(compiled.sql).toContain(
    ...
    expect(compiled.sql).toContain('"journal_entries"."visibility" = $9');
    expect(compiled.sql).toContain('"journal_entries"."lifecycle_state" = $10');
    expect(compiled.sql).toContain(
    ...
    expect(compiled.sql).toContain('"public_media"."original_deleted_at" is not null');
  ...
  it("selects bounded entry readback with derivative media only", () => {
    ...
    expect(compiled.sql).toContain('"catalog_items"."public_slug" = $6');
    expect(compiled.sql).toContain('"catalog_items"."status" in ($7, $8)');
    expect(compiled.sql).not.toContain("provisional");
    expect(compiled.sql).not.toContain("rejected");
    expect(compiled.sql).not.toContain("merged");
    expect(compiled.sql).toContain(
    ...
    expect(compiled.sql).toContain('"plant_objects"."variety_state" = $9');
    expect(compiled.sql).toContain('"journal_entries"."visibility" = $10');
    expect(compiled.sql).toContain('"journal_entries"."lifecycle_state" = $11');
    expect(compiled.sql).toContain(
      '"journal_entries"."public_gone_at" is null',
    );
    expect(compiled.sql).toContain(
    ...
    expect(compiled.sql).not.toContain("quarantine_key");
    expect(compiled.sql).toContain(
      '"media_assets"."original_deleted_at" is not null',
    );
